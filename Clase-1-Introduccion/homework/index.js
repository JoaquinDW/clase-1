// Todos los ejercicios tienen su consigna,
// si tienes alguna duda sobre el enunciado, no dudes en preguntar en el canal de Discord
// Recuerda que puedes correr tus funciones con el comando `npm run test` para verificar si son correctas
// Ademas tienes material de apoyo en el archivo README.md

export function imprimirMensaje(mensaje) {
  // Escribe aquí el código para imprimir un mensaje en la consola
  // La función debe recibir un mensaje como parámetro y mostrarlo en la consola
}

export function suma(a, b) {
  // 'a' y 'b' son números, sumalos y devolve su valor
  //tu codigo
}

export function resta(a, b) {
  // 'a' y 'b' son números, restalos y devolve su valor
  //tu codigo
}

export function multiplicacion(a, b) {
  // 'a' y 'b' son números, multiplicarlos y devolve su valor
  //tu codigo
}

export function iguales(a, b) {
  // 'a' y 'b' son números, devolver si son iguales o no
  //tu codigo
}

export function calcularAreaTriangulo(base, altura) {
  // Escribe aquí el código para calcular el área de un triángulo
  // La función debe recibir 'base' y 'altura' como parámetros y retornar el área calculada
  // Psst: Como se calcula el area de un triangulo?
}

export function manipularObjetoBasico() {
  // Escribe aquí el código para manipular un objeto básico en JavaScript
  // Tienes un objeto 'producto' con propiedades como nombre, precio, cantidad, etc.
  // Realiza operaciones como agregar una nueva propiedad 'categoria' y eliminar la propiedad 'cantidad'
  // La función debe retornar el objeto modificado
  let producto = {
    nombre: "Producto",
    precio: 100,
    cantidad: 10,
  }
  // tu codigo
  return producto
}

export function manipularArregloBasico() {
  // Escribe aquí el código para manipular un arreglo básico en JavaScript
  // Define un arreglo llamado 'frutas' con elementos como manzana, pera, banana, tomate.
  // Realiza operaciones como agregar un nuevo elemento 'uva' y eliminar el elemento 'tomate'
  // La función debe retornar el arreglo modificado
}

export function splitString(string) {
  // Escribe aquí el código para dividir un string en un arreglo
  // La función debe recibir un string y retornar un arreglo con cada palabra del string
  // Psst: Recuerda que puedes usar el método split
}
